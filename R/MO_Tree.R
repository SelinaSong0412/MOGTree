...
#' @param mus.hat Optional list of estimated counterfactual mean matrices, one
#'   per outcome.
...
#' @param w_vec A matrix of weight vectors; each row defines one outcome-weight
#'   combination used in the gain calculation.
#' @param weight_combine Rule used to aggregate gain values across the rows of
#'   `w_vec`. Current options are `"max"` and `"mean"`.
#' @param lambda Minimum gain required to continue splitting a node.
#'
...
#   if (is.null(Ys)) {
#     # print("Please input observed ourcome as a metrix.")
#     if (is.null(mus.hat)) {
#       print("Please input observed ourcome as a metrix.")
#     } else {
#     mus.hat = mus.hat
#     n<-nrow(mus.hat[[1]]) #number of people
#     n.obj<-length(mus.hat) # number of outcomes
...
DTRtree<-function(Ys,
  ...
  colnames(output)<-c("node","X","cutoff","mEy","group")
  # indicator for subset data
  # YS update the code for calculating mus.hat for multiple objs
  if (!is.null(mus.hat)) {
    mus.hat = mus.hat
  } else {
      mus.hat <- lapply(1:n.obj, function(i) matrix(0, nrow = n, ncol = length(class.A)))
