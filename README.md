...
3.  estimate propensity scores with `M.propen()` and outcome regressions
    with `Reg.mu()`;
4.  summarize leaf-specific treatment preferences across a grid of
    outcome weights with `mus.AIPW()`.

The current implementation matches the functions used in the
repository’s simulation studies. In particular, the tree fit returns the
subgroup structure, and the weight-specific treatment summaries are
built from the AIPW estimates.
