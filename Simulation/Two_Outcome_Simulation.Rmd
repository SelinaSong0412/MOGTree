...
library(MOTRL)
library(ggsci)
library(forcats) # For factor manipulation (fct_recode)
...
method_colors_for_plot <- c(
  "GMM" = "#f7766d",      # Your specified color for GMM
  "HC" = "#c49a00",# "#efce38",      # Your specified color for HC
  # "KMeans3" is removed
  "KMeans" =  "#02c094", # "#54b300", # Your specified color for KMeans4, now named "KMeans"
  "MOTree" = "#00b6eb",   # Your specified color for MOTree
  "SOM" = "#a489ff",      # Your specified color for SOM
  "Spectral" = "#F781BF"  # Your specified color for Spectral
)
  ...
  for (i in c(1:iter)) {
    ...
    for (method in seq_along(clustering_methods)) {
      ...
      for (w_idx in seq_along(weights)) {
          ...
          avg_cf_results[[method]][i] <- mean(df_obs[[paste0("cf_", method_name, "_w", w_idx - 1)]], na.rm = T)
...
```{r}
# smaller sample size
result10 = Simulation_MOTree(iter = 100, n.cov = 5, n.per.group = 10)

# smaller sample size
result25 = Simulation_MOTree(iter = 100, n.cov = 5, n.per.group = 25)
...
# larger sample size
result150 = Simulation_MOTree(iter = 100, n.cov = 5, n.per.group = 150)

# larger sample size
result200 = Simulation_MOTree(iter = 100, n.cov = 5, n.per.group = 200)


save(result10, result25, result50, result100, result150, result200, file = "Two_obj_results.RData")
```


```{r}
result10$clust_perform
result25$clust_perform
result50$clust_perform
result100$clust_perform
result150$clust_perform
```
...
```{r}
result200$clust_perform
result200$cf_Y_perform
result200$non_dominant_perform
```
...
```{r}
outcomes <- result25$cf_Y  # Counterfactual outcomes
true_opt_means <- result25$true_optimal_weighted_means  # 1x11 matrix of observed weighted outcomes
...
outcomes_filtered_renamed <- outcomes_long %>%
  filter(Method != "KMeans3") %>% # Remove rows for KMeans3
  mutate(
    Method = fct_recode(Method,
      "KMeans" = "KMeans4" # Rename KMeans4 to KMeans
    )
  ) %>%
  mutate(Method = droplevels(Method))

ggplot(outcomes_filtered_renamed, aes(x = Weight, y = Difference, color = Method, fill = Method)) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2) +  
  labs(title = "Difference between Estimated and True Optimal Counterfactual Outcome",
       x = "Weight on Y1",
  ...
  scale_color_manual(values = method_colors_for_plot) +
  scale_fill_manual(values = method_colors_for_plot) # +
...
ggplot(outcomes_long, aes(x = Weight, y = Difference, color = Method, fill = Method)) +
  ...
  scale_color_discrete(labels = c(
    "GMM" = "GMM",
    "HC" = "HC",
    "KMeans3" = "KMeans-TG", # Renamed
    "KMeans4" = "KMeans-G",  # Renamed
    "MOTree" = "MOTree",
    "SOM" = "SOM",
    "Spectral" = "Spectral"
  )) +
  ...
  # scale_color_d3() + scale_fill_d3() # its okay, looks clear and our method is red
  # scale_color_jama()  + scale_fill_jama()
  guides(color = guide_legend(nrow = 1, byrow = TRUE)) # Arrange color legend items in 1 row

# scales::show_col(pal_npg("nrc", alpha = 0.7)(7))

# c("#f7766d", "#c49a00", "#53b401", "#02c094", "#00b6eb", "#a58aff", "#fb61d7")
```

Plot the differences (Delete KMeans3)
```{r}
outcomes_filtered_renamed <- outcomes_long %>%
  filter(Method != "KMeans3") %>% # Remove rows for KMeans3
  mutate(
    Method = fct_recode(Method,
      "KMeans" = "KMeans4" # Rename KMeans4 to KMeans
      # Other methods will keep their original names
    )
  ) %>%  
  mutate(Method = droplevels(Method))
...
```{r}
# Extract outcomes from the simulation results
outcomes <- result100$cf_Y  # Counterfactual outcomes
true_opt_means <- result100$true_optimal_weighted_means  # 1x11 matrix of observed weighted outcomes
...
```{r}
# Extract the non-dominant data from the simulation results
non_dominant <- result100$non_dominant
